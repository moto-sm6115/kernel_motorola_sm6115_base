...
static struct snd_soc_dai_link msm_fs16xx_be_dai_links[] = {
	{
		.name = LPASS_BE_PRI_MI2S_RX,
		.stream_name = "Primary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.0",
...
#if !defined(CONFIG_SND_SMARTPA_AW882XX) && !defined(CONFIG_SND_SOC_FS16XX)
...
#endif
#ifdef CONFIG_SND_SOC_FS16XX
			ARRAY_SIZE(msm_fs16xx_be_dai_links) +
#endif
...
#ifdef CONFIG_SND_SOC_FS16XX
		rc = of_property_read_u32(dev->of_node, "fs,have-16xx",
			&has_fs16xx_pa);
		if (rc) {
			dev_dbg(dev, "%s: No DT match Fs16xx audio interface\n",
			__func__);
		} else {
			dev_dbg(dev,"%s,fs,has-16xx=%d\n",__func__,has_fs16xx_pa);
			if (has_fs16xx_pa) {
				memcpy(msm_bengal_dai_links + total_links,
					msm_fs16xx_be_dai_links,
					sizeof(msm_fs16xx_be_dai_links));
				total_links +=
				ARRAY_SIZE(msm_fs16xx_be_dai_links);
			}
		}
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (!gpio_is_valid(pdata->ext_hac_gpio)) {
		...
		ret = gpio_request(pdata->ext_hac_gpio,
			"ext_hac_gpio");
		dev_dbg(&pdev->dev, "qcom,ext-hac-gpio = %d", pdata->ext_hac_gpio);
	}
